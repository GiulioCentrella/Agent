...
LPTSTR lpszHKCRShort = TEXT("HKCR");
LPTSTR lpszHKCRLong  = TEXT("HKEY_CLASSES_ROOT");
#define CCH_HKCR_LONG  18

LPTSTR lpszHKCUShort = TEXT("HKCU");
LPTSTR lpszHKCULong  = TEXT("HKEY_CURRENT_USER");
#define CCH_HKCU_LONG  18
...
LPTSTR lpszHKCCShort = TEXT("HKCC");
LPTSTR lpszHKCCLong  = TEXT("HKEY_CURRENT_CONFIG");
#define CCH_HKCC_LONG  19
    ...
    for (lpKC = lpVC->lpFatherKC; NULL != lpKC; lpKC = lpKC->lpFatherKC) {
        if (NULL != lpKC->lpszKeyName) {
            if ((fUseLongNames) && (lpszHKLMShort == lpKC->lpszKeyName)) {
                ...
                cchName += CCH_HKU_LONG;
            } else if ((fUseLongNames) && (lpszHKCCShort == lpKC->lpszKeyName)) {
                cchName += CCH_HKCC_LONG;
            } else if ((fUseLongNames) && (lpszHKCRShort == lpKC->lpszKeyName)) {
                cchName += CCH_HKCR_LONG;
    ...
    for (lpKC = lpVC->lpFatherKC; NULL != lpKC; lpKC = lpKC->lpFatherKC) {
        if (NULL != lpKC->lpszKeyName) {
            if ((fUseLongNames) && (lpszHKLMShort == lpKC->lpszKeyName)) {
                ...
                cchName = CCH_HKCC_LONG;
                lpszKeyName = lpszHKCCLong;
            } else if ((fUseLongNames) && (lpszHKCRShort == lpKC->lpszKeyName)) {
                cchName = CCH_HKCR_LONG;
                lpszKeyName = lpszHKCRLong;
            } else if ((fUseLongNames) && (lpszHKCUShort == lpKC->lpszKeyName)) {
                cchName = CCH_HKCU_LONG;
...
// ----------------------------------------------------------------------
LPTSTR TransData(LPVALUECONTENT lpVC, DWORD nConversionType, BOOL fForOutput)
{
        ...
        switch (nConversionType) {
            ...
            default:
                ...
                if (cbData != lpVC->cbData) {
                    _sntprintf(lpszValueData + (1 + (ibCurrent * 3)), 4, TEXT(" ...\0"));
                }
        }
    }
...
// ----------------------------------------------------------------------
LPTSTR GetWholeValueData(LPVALUECONTENT lpVC, BOOL fForOutput)
{
    LPTSTR lpszValueData;
    ...
    if (NULL == lpVC->lpValueData) {
        lpszValueData = TransData(lpVC, REG_BINARY, fForOutput);
    } else {
        DWORD cbData;
        size_t cchMax;
        size_t cchActual;
        ...
        switch (lpVC->nTypeCode) {
            ...
            case REG_MULTI_SZ:
                ...
                if ((cchActual * sizeof(TCHAR)) == cbData) {
                    lpszValueData = TransData(lpVC, lpVC->nTypeCode, fForOutput);
                } else {
                    lpszValueData = TransData(lpVC, REG_BINARY, fForOutput);
                }
                break;
...
size_t ResultToString(LPTSTR rgszResultStrings[], size_t iResultStringsMac, DWORD nActionType, LPVOID lpContent, BOOL fNewContent)
{
    ...
    if ((KEYDEL == nActionType) || (KEYADD == nActionType)) {
        ...
        // data
        lpszData = GetWholeValueData(lpContent, TRUE);
        cchData = 0;
        if (NULL != lpszData) {
...
VOID CompareShots(VOID)
{
    ...
    ClearRegKeyMatchFlags(Shot1.lpHKCC);
    ClearRegKeyMatchFlags(Shot2.lpHKCC);
    ClearRegKeyMatchFlags(Shot1.lpHKCR);
    ClearRegKeyMatchFlags(Shot2.lpHKCR);
    ...
    // Compare HKCR
    if ((NULL != CompareResult.lpShot1->lpHKCR) || (NULL != CompareResult.lpShot2->lpHKCR)) {
    ...
    // Compare HKCC
       if ((NULL != CompareResult.lpShot1->lpHKCC) || (NULL != CompareResult.lpShot2->lpHKCC)) {
...
BOOL OutputComparisonResult(VOID)
{
    ...
    if (INVALID_HANDLE_VALUE == hFile) {
    	...
    	hFile = CreateFile(lpszDestFileName, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    }
    ...
    //WriteTitle(TEXT("Datetime:"), lpszBuffer, fAsHTML);
    ...
    //WriteTitle(TEXT("Computer:"), lpszBuffer, fAsHTML);
    ...
    //WriteTitle(TEXT("Username:"), lpszBuffer, fAsHTML);

    MYFREE(lpszBuffer);

    // Write keydel part
    if (0 != CompareResult.stcDeleted.cKeys) {
        WriteTableHead(TEXT("Keys deleted:"), CompareResult.stcDeleted.cKeys, fAsHTML);
        WritePart(KEYDEL, CompareResult.stCRHeads.lpCRKeyDeleted, fAsHTML, TRUE);
    }
    // Write keyadd part
    if (0 != CompareResult.stcAdded.cKeys) {
        WriteTableHead(TEXT("Keys added:"), CompareResult.stcAdded.cKeys, fAsHTML);
        WritePart(KEYADD, CompareResult.stCRHeads.lpCRKeyAdded, fAsHTML, TRUE);
    }
    // Write valdel part
    if (0 != CompareResult.stcDeleted.cValues) {
        WriteTableHead(TEXT("Values deleted:"), CompareResult.stcDeleted.cValues, fAsHTML);
        WritePart(VALDEL, CompareResult.stCRHeads.lpCRValDeleted, fAsHTML, TRUE);
    }
    // Write valadd part
    if (0 != CompareResult.stcAdded.cValues) {
        WriteTableHead(TEXT("Values added:"), CompareResult.stcAdded.cValues, fAsHTML);
        WritePart(VALADD, CompareResult.stCRHeads.lpCRValAdded, fAsHTML, TRUE);
    }
    // Write valmodi part
    if (0 != CompareResult.stcModified.cValues) {
        WriteTableHead(TEXT("Values modified:"), CompareResult.stcModified.cValues, fAsHTML);
    ...
    if (0 != CompareResult.stcDeleted.cDirs) {
        WriteTableHead(TEXT("Folders deleted:"), CompareResult.stcDeleted.cDirs, fAsHTML);
        WritePart(DIRDEL, CompareResult.stCRHeads.lpCRDirDeleted, fAsHTML, TRUE);
    }
    // Write directory add part
    if (0 != CompareResult.stcAdded.cDirs) {
        WriteTableHead(TEXT("Folders added:"), CompareResult.stcAdded.cDirs, fAsHTML);
        WritePart(DIRADD, CompareResult.stCRHeads.lpCRDirAdded, fAsHTML, TRUE);
    }
    // Write directory modi part
    if (0 != CompareResult.stcModified.cDirs) {
        WriteTableHead(TEXT("Folders attributes changed:"), CompareResult.stcModified.cDirs, fAsHTML);
        WritePart(DIRMODI, CompareResult.stCRHeads.lpCRDirModified, fAsHTML, TRUE);
    }
    // Write file del part
    if (0 != CompareResult.stcDeleted.cFiles) {
        WriteTableHead(TEXT("Files deleted:"), CompareResult.stcDeleted.cFiles, fAsHTML);
        WritePart(FILEDEL, CompareResult.stCRHeads.lpCRFileDeleted, fAsHTML, TRUE);
    }
    // Write file add part
    if (0 != CompareResult.stcAdded.cFiles) {
        WriteTableHead(TEXT("Files added:"), CompareResult.stcAdded.cFiles, fAsHTML);
        WritePart(FILEADD, CompareResult.stCRHeads.lpCRFileAdded, fAsHTML, TRUE);
    }
    // Write file modi part
    if (0 != CompareResult.stcModified.cFiles) {
        WriteTableHead(TEXT("Files [attributes?] modified:"), CompareResult.stcModified.cFiles, fAsHTML);
        WritePart(FILEMODI, CompareResult.stCRHeads.lpCRFileModified, fAsHTML, TRUE);
    }
    WriteTableHead(TEXT("Total changes:"), CompareResult.stcChanged.cAll, fAsHTML);
    ...
    /*if (32 >= (size_t)ShellExecute(hWnd, TEXT("open"), lpszDestFileName, NULL, NULL, SW_SHOW)) {
        ErrMsg(TEXT("Error call External Viewer!"));
    }*/
        ...
        if (NULL != lpKC->lpszKeyName) {
            if ((NULL != lpKC->lpFatherKC)  // only the top KC can have HKLM/HKU, so ignore the sub KCs
                    || ((lpKC->lpszKeyName != lpszHKLMShort)
						...
						&& (lpKC->lpszKeyName != lpszHKCRShort)
						&& (lpKC->lpszKeyName != lpszHKCRLong)
						&& (lpKC->lpszKeyName != lpszHKCUShort)
						&& (lpKC->lpszKeyName != lpszHKCULong)
						&& (lpKC->lpszKeyName != lpszHKCCShort)
						&& (lpKC->lpszKeyName != lpszHKCCLong))) {
    ...
    // Extra local block to reduce stack usage due to recursive calls
    {
        ...
        if (0 < cValues) {
            ...
            for (i = 0; ; i++) {
                ...
                lpszDebugMsg = GetWholeValueData(lpVC, FALSE);
                DebugLog(lpszDebugValueNameDataLog, lpszDebugMsg, TRUE);
                MYFREE(lpszDebugMsg);
#endif
...
VOID Shot(LPREGSHOT lpShot)
{
   //DWORD cchString;
    ...
    // Take HKCR registry shot
    //GetRegistrySnap(lpShot, HKEY_CLASSES_ROOT, lpszHKCRShort, NULL, &lpShot->lpHKCR);
    ...
    // Take HKCC registry shot
    GetRegistrySnap(lpShot, HKEY_CURRENT_CONFIG, lpszHKCCShort, NULL, &lpShot->lpHKCC);
            ...
            if (NULL != lpszKeyName) {
                ...
                if ((fUseLongRegHead) && (0 == nFPFatherKey)) {
                    ...
                    if (lpszHKLMShort == lpszKeyName) {
                        ...
                        lpszKeyName = lpszHKULong;
                        sKC.nKeyNameLen = (DWORD)_tcslen(lpszKeyName);;
                    } else if (lpszHKCCShort == lpszKeyName) {
                        lpszKeyName = lpszHKCCLong;
                        sKC.nKeyNameLen = (DWORD)_tcslen(lpszKeyName);;
                    } else if (lpszHKCRShort == lpszKeyName) {
                        lpszKeyName = lpszHKCRLong;
                        sKC.nKeyNameLen = (DWORD)_tcslen(lpszKeyName);;
...
// Save registry and files to new Reg_Shot
...
VOID SaveShot(LPREGSHOT lpShot)
{
    ...
    if ((NULL == lpShot->lpHKCR) && (NULL == lpShot->lpHKCU) && (NULL == lpShot->lpHKLM) && (NULL == lpShot->lpHKU) && (NULL == lpShot->lpHKCC) && (NULL == lpShot->lpHF)) {
    ...
    hFileWholeReg = CreateFile(pathShot, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    fileheader.ofsHKCC = 0;
    fileheader.ofsHKCR = 0;
    ...
    // Save HKCR
    if (NULL != lpShot->lpHKCR) {
            SaveRegKeys(lpShot, lpShot->lpHKCR, 0, offsetof(FILEHEADER, ofsHKCR));
    }

    // Save HKCU
    if (NULL != lpShot->lpHKCU) {
        SaveRegKeys(lpShot, lpShot->lpHKCU, 0, offsetof(FILEHEADER, ofsHKCU));
    }
    ...
    // Save HKCC
       if (NULL != lpShot->lpHKCC) {
           SaveRegKeys(lpShot, lpShot->lpHKCC, 0, offsetof(FILEHEADER, ofsHKCC));
       }
    ...
    for (; 0 != ofsKey; ofsKey = ofsBrotherKey) {
        ...
        if (NULL == lpKC->lpFatherKC) {
            if ((0 == _tcscmp(lpKC->lpszKeyName, lpszHKLMShort))
                ...
                lpKC->lpszKeyName = lpszHKUShort;
                lpKC->cchKeyName = _tcslen(lpKC->lpszKeyName);
            }else if ((0 == _tcscmp(lpKC->lpszKeyName, lpszHKCCShort))
                    || (0 == _tcscmp(lpKC->lpszKeyName, lpszHKCCLong))) {
             MYFREE(lpKC->lpszKeyName);
             lpKC->lpszKeyName = lpszHKCCShort;
             lpKC->cchKeyName = _tcslen(lpKC->lpszKeyName);
            } else if ((0 == _tcscmp(lpKC->lpszKeyName, lpszHKCRShort))
                    || (0 == _tcscmp(lpKC->lpszKeyName, lpszHKCRLong))) {
             MYFREE(lpKC->lpszKeyName);
             lpKC->lpszKeyName = lpszHKCRShort;
             lpKC->cchKeyName = _tcslen(lpKC->lpszKeyName);
...
// Load registry and files from old Reg_Shot
...
BOOL LoadShot(LPREGSHOT lpShot)
{
    ...
    hFileWholeReg = CreateFile(pathShot, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    if (INVALID_HANDLE_VALUE == hFileWholeReg) {
    ...
    if (FILEHEADER_VERSION_EMPTY == fileheader.nFHVersion) {
        ...
        if ((0 != fileheader.ofsHKCR) && (fileheader.ofsHF == fileheader.ofsHKCR)) {
            fileheader.ofsHKCR = 0;
        }
        if ((0 != fileheader.ofsHKCC) && (fileheader.ofsHF == fileheader.ofsHKCC)) {
                    fileheader.ofsHKCC = 0;
                }
    ...
    if (0 != fileheader.ofsHKCC) {
            LoadRegKeys(lpShot, fileheader.ofsHKCC, NULL, &lpShot->lpHKCC);
        }

    if (0 != fileheader.ofsHKCR) {
            LoadRegKeys(lpShot, fileheader.ofsHKCR, NULL, &lpShot->lpHKCR);
        }
